...
class AsyncFragmentAPI:
    ...
    async def buy_stars(self, username: str, quantity: int, show_sender: bool = True) -> Any:
        ...
        try:
            ...
            # Jika gagal dengan JSON, coba dengan form data
            if response.status_code != 200 or 'error' in response.text.lower():
                print("🔄 Mencoba dengan format form data...")
                ...
                print(f"📥 Response status (form): {response.status_code}")
                print(f"📦 Response text (form): {response.text[:500]}")
            ...
            try:
                ...
                # Jika bukan JSON, cek apakah response sukses
                if 'success' in response.text.lower() or 'ok' in response.text.lower():
                    return Result(True, tx_hash=f"TX_{int(time.time())}")
